document.addEventListener("DOMContentLoaded", function() {
    document.querySelector('.chevron-down-haircare').addEventListener('click', function() {
        const filterContainer = document.querySelector('.filter-container-haircare');

        // Check if the filter container is currently visible
        const isVisible = filterContainer.classList.contains('filter-container-show-haircare');
        
        console.log("Is filter container visible?", isVisible); // Debugging log

        // Toggle rotation class
        this.classList.toggle('rotate', !isVisible); // Add/remove rotation class

        // Show or hide the filter container
        filterContainer.classList.toggle('filter-container-show-haircare'); // Toggle the filter container visibility

        // Check if the class is added/removed
        console.log("Current classes:", filterContainer.classList);
    });
});
